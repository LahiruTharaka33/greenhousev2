...
export default function ConfigurationPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // First, save the client ID mappings
      const response = await fetch('/api/configuration/mappings', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        // Save tank configurations in parallel for better performance
        const tankConfigs = [
          { tankName: 'Tank A', value: tankA },
          { tankName: 'Tank B', value: tankB },
          { tankName: 'Tank C', value: tankC },
        ];
  ...
  const handleEditMapping = (customerId: string, tunnelId: string, currentClientId: string | null, currentSensorClientId: string | null, tankConfigs: TankConfig[]) => {
    ...
    const tankBConfig = tankConfigs.find(config => config.tankName === 'Tank B');
    const tankCConfig = tankConfigs.find(config => config.tankName === 'Tank C');
    
    setTankA(tankAConfig ? (tankAConfig.itemType === 'water' ? 'water' : tankAConfig.itemId || '') : '');
    setTankB(tankBConfig ? (tankBConfig.itemType === 'water' ? 'water' : tankBConfig.itemId || '') : '');
    setTankC(tankCConfig ? (tankCConfig.itemType === 'water' ? 'water' : tankCConfig.itemId || '') : '');
                  ...
                  <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="border-t border-gray-200 pt-4 mt-4">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                          <label htmlFor="tankA" className="block text-sm font-medium text-gray-700 mb-1">
                            Tank A
                          </label>
                          <select
                            id="tankA"
                        ...
                        <div>
                          <label htmlFor="tankB" className="block text-sm font-medium text-gray-700 mb-1">
                            Tank B
                          </label>
                          <select
                            id="tankB"
                        ...
                        <div>
                          <label htmlFor="tankC" className="block text-sm font-medium text-gray-700 mb-1">
                            Tank C
                          </label>
                          <select
                            id="tankC"
                            value={tankC}
                            onChange={(e) => setTankC(e.target.value)}
                            disabled={loadingItems}
                            className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 disabled:bg-gray-100 disabled:cursor-not-allowed"
                          >
                            <option value="">Select...</option>
                          ...
                          {mapping.tunnels.map((tunnel) => (
                            <div key={tunnel.id} className="border border-gray-200 rounded-lg p-4 bg-gray-50">
                               <div className="space-y-3">
                                 ...
                                 {/* Tank Configurations - Always visible */}
                                 <div className="mt-3 pt-3 border-t border-gray-300">
                                   ...
                                   <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pl-8">
                                     {['Tank A', 'Tank B', 'Tank C'].map((tankName) => {
                                       ...
                                       return (
                                         <div key={tankName} className="flex items-center space-x-2">
                                           <span className="text-xs font-medium text-gray-600 min-w-[60px]">{tankName}:</span>
                                           {existingConfig ? (
                                             <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-blue-100 text-blue-800">
                                               {existingConfig.itemType === 'water' ? 'Water' : existingConfig.item?.itemName || 'Unknown'}
                                             </span>
                                           ) : (
